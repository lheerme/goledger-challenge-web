...
import { EditArtistDialogForm } from "./edit-artist-dialog-form";
...
export function EditArtistDialog() {
  return (
    <Dialog.Portal>
      ...
      >
        <Dialog.Title className="text-lg font-medium leading-none">Editar artista</Dialog.Title>
        <Dialog.Close className="absolute right-6 top-2 hover:text-violet-400 transition-colors">
          <X className="size-4" />
        </Dialog.Close>
        <EditArtistDialogForm />
      </Dialog.Content>
    </Dialog.Portal>
  )
}